"""
Optimize ML models for deployment on memory-constrained environments (Render free tier: 512MB RAM)
This script creates lightweight versions of TabNet models with reduced memory footprint.
"""

import torch
import numpy as np
from pytorch_tabnet.tab_model import TabNetClassifier, TabNetRegressor
from pathlib import Path
import pickle
import gc

MODEL_DIR = Path(__file__).parent / 'optimized_models'

def optimize_tabnet_model(model_path, model_type='classifier'):
    """
    Optimize a TabNet model by:
    1. Loading with CPU-only mode
    2. Converting to half precision (FP16) where possible
    3. Removing unnecessary data
    4. Re-saving with minimal overhead
    """
    print(f"\nOptimizing {model_path}...")
    
    try:
        # Load model
        if model_type == 'classifier':
            model = TabNetClassifier()
        else:
            model = TabNetRegressor()
        
        model.load_model(str(model_path))
        
        # Get the model's network
        network = model.network
        
        # Set to eval mode and move to CPU
        network.eval()
        network.cpu()
        
        # Convert model weights to FP16 for memory efficiency
        # Note: Keep input/output as FP32 for compatibility
        for param in network.parameters():
            if param.requires_grad:
                param.requires_grad = False
            # Convert to half precision
            param.data = param.data.half()
        
        # Save optimized model
        optimized_path = str(model_path).replace('.zip', '_optimized.zip')
        model.save_model(optimized_path)
        
        print(f"✓ Optimized model saved to: {optimized_path}")
        
        # Clean up
        del model
        del network
        gc.collect()
        
        return optimized_path
        
    except Exception as e:
        print(f"✗ Failed to optimize {model_path}: {e}")
        return None


def create_lightweight_loader():
    """
    Create a lightweight model loader that uses lazy loading and memory optimization
    """
    loader_code = '''
"""
Lightweight ML model loader optimized for low-memory environments
"""
import torch
import gc
from pytorch_tabnet.tab_model import TabNetClassifier, TabNetRegressor
from pathlib import Path

# Force CPU-only mode to reduce memory
torch.set_num_threads(1)

class LightweightMLModels:
    """Memory-optimized model loader with lazy loading"""
    _instance = None
    
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(LightweightMLModels, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance
    
    def _load_models(self):
        """Load models with memory optimization"""
        if self._initialized:
            return
        
        print("Loading optimized ML models...")
        MODEL_DIR = Path(__file__).parent / 'optimized_models'
        
        # Load models one at a time and clear memory between loads
        try:
            self.obesity_model = TabNetClassifier()
            model_path = MODEL_DIR / 'obesity' / 'obesity_tabnet_best_optimized.zip'
            if not model_path.exists():
                model_path = MODEL_DIR / 'obesity' / 'obesity_tabnet_best.zip'
            self.obesity_model.load_model(str(model_path))
            self.obesity_model.network.eval()
            self.obesity_classes = ['Insufficient_Weight', 'Normal_Weight', 'Obesity_Type_I', 
                                   'Obesity_Type_II', 'Obesity_Type_III', 'Overweight_Level_I', 
                                   'Overweight_Level_II']
            print("✓ Obesity model loaded")
            gc.collect()
        except Exception as e:
            print(f"✗ Obesity model failed: {e}")
            self.obesity_model = None
        
        try:
            self.exercise_model = TabNetRegressor()
            model_path = MODEL_DIR / 'exercise' / 'exercise_tabnet_best_optimized.zip'
            if not model_path.exists():
                model_path = MODEL_DIR / 'exercise' / 'exercise_tabnet_best.zip'
            self.exercise_model.load_model(str(model_path))
            self.exercise_model.network.eval()
            print("✓ Exercise model loaded")
            gc.collect()
        except Exception as e:
            print(f"✗ Exercise model failed: {e}")
            self.exercise_model = None
        
        try:
            self.menstrual_model = TabNetClassifier()
            model_path = MODEL_DIR / 'menstrual' / 'menstrual_tabnet_best_optimized.zip'
            if not model_path.exists():
                model_path = MODEL_DIR / 'menstrual' / 'menstrual_tabnet_best.zip'
            self.menstrual_model.load_model(str(model_path))
            self.menstrual_model.network.eval()
            self.menstrual_classes = ['Regular', 'Short', 'Long']
            print("✓ Menstrual model loaded")
            gc.collect()
        except Exception as e:
            print(f"✗ Menstrual model failed: {e}")
            self.menstrual_model = None
        
        self._initialized = True
        print("Model loading complete!")
    
    def get_obesity_model(self):
        if not self._initialized:
            self._load_models()
        return self.obesity_model
    
    def get_exercise_model(self):
        if not self._initialized:
            self._load_models()
        return self.exercise_model
    
    def get_menstrual_model(self):
        if not self._initialized:
            self._load_models()
        return self.menstrual_model
'''
    
    with open(MODEL_DIR.parent / 'lightweight_models.py', 'w') as f:
        f.write(loader_code)
    
    print("\n✓ Created lightweight_models.py")


def main():
    """Optimize all models"""
    print("=" * 60)
    print("ML Model Optimization for Render Free Tier (512MB RAM)")
    print("=" * 60)
    
    models_to_optimize = [
        (MODEL_DIR / 'obesity' / 'obesity_tabnet_best.zip', 'classifier'),
        (MODEL_DIR / 'exercise' / 'exercise_tabnet_best.zip', 'regressor'),
        (MODEL_DIR / 'menstrual' / 'menstrual_tabnet_best.zip', 'classifier'),
    ]
    
    optimized_count = 0
    for model_path, model_type in models_to_optimize:
        if model_path.exists():
            result = optimize_tabnet_model(model_path, model_type)
            if result:
                optimized_count += 1
        else:
            print(f"✗ Model not found: {model_path}")
    
    print(f"\n{'=' * 60}")
    print(f"Optimization complete: {optimized_count}/{len(models_to_optimize)} models optimized")
    print(f"{'=' * 60}")
    
    # Create lightweight loader
    create_lightweight_loader()
    
    print("\nNext steps:")
    print("1. Update fastapi_server.py to use lightweight_models.py")
    print("2. Test locally that models still work")
    print("3. Deploy to Render")


if __name__ == '__main__':
    main()
